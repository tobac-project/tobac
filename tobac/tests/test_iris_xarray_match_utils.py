...
def test_add_coordinates_xarray_base(
    ...
    xr_coord_interp["time"] = datetime_utils.match_datetime_format(xr_coord_interp.time, iris_coord_interp.time)
...
def test_preserve_iris_datetime_types():
    ...
    xarray_output = xr_utils.add_coordinates_to_features(
        all_feats, convert_cube_to_dataarray(var_array, preserve_iris_datetime_types=True)
    )
    ...
    xarray_output_datetime_preserve_off = xr_utils.add_coordinates_to_features(
        all_feats, convert_cube_to_dataarray(var_array, preserve_iris_datetime_types=False)
    )
