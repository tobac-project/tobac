...
def test_to_cftime():
    ...
    for date in test_dates:
        ...
        assert datetime_utils.to_cftime(date, "gregorian") == cftime.DatetimeGregorian(2000,1,1)
        assert datetime_utils.to_cftime(date, "360_day") == cftime.Datetime360Day(2000,1,1)
        assert datetime_utils.to_cftime(date, "365_day") == cftime.DatetimeNoLeap(2000,1,1)
...
def test_to_datetime64():
    ...
    for date in test_dates:
        assert datetime_utils.to_datetime64(date) == np.datetime64("2000-01-01 00:00:00.000000000")
...
def test_to_datestr():
    ...
    for date in test_dates:
        assert datetime_utils.to_datestr(date) == "2000-01-01T00:00:00.000000000" or datetime_utils.to_datestr(date) == "2000-01-01T00:00:00"
...
def test_match_datetime_format():
    ...
    for target in test_dates:
        for date in test_dates:
            assert datetime_utils.match_datetime_format(
                date, target
            ) == target