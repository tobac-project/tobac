...
def test_field_and_features_over_time():
    ...
    assert_frame_equal(iter_0[3], test_features[test_features.time==datetime(2000, 1, 1)])
    ...
    assert_frame_equal(iter_1[3], test_features[test_features.time==datetime(2000, 1, 1, 1)])
...
def test_field_and_features_over_time_time_padding():
    ...
    _, _, _, df_slice = next(generators.field_and_features_over_time(test_data, test_features))

    assert len(df_slice) == 1
    assert_frame_equal(df_slice, test_features.loc[0:0])

    # Test time padding of 1 second
    _, _, _, df_slice = next(generators.field_and_features_over_time(test_data, test_features, time_padding=timedelta(seconds=1)))

    assert len(df_slice) == 2
    assert_frame_equal(df_slice, test_features.loc[0:1])

    # Test time padding of 2 seconds
    _, _, _, df_slice = next(generators.field_and_features_over_time(test_data, test_features, time_padding=timedelta(seconds=2)))
...
def test_field_and_features_over_time_cftime():
    test_data = xr.DataArray(
        np.zeros([2,10,10]),
        dims=("time","y","x"), 
        coords={"time":[cftime.Datetime360Day(2000,1,1), cftime.Datetime360Day(2000,1,1,1)]}
    )
    ...
    iterator = generators.field_and_features_over_time(test_data, test_features, time_padding=timedelta(seconds=1))
    ...
    assert np.all(iter_1[2] == test_data.isel(time=1))
    assert_frame_equal(iter_1[3], test_features[test_features.time==cftime.Datetime360Day(2000,1,1,1)])
