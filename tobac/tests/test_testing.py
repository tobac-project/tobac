...
'''
import pytest
from tobac.testing import get_pbc_coordinates, generate_single_feature, get_single_pbc_coordinate
...
def test_make_feature_blob():
    ...
    # 2D test
    out_blob = tbtest.make_feature_blob(np.zeros((10,10)), h1_loc=5, h2_loc=5, 
        h1_size = 2, h2_size= 2, shape='rectangle', amplitude= 1, PBC_flag='none')
    ...
    # 3D test
    out_blob = tbtest.make_feature_blob(np.zeros((10, 10, 10)), h1_loc=5, h2_loc=5, 
                v_loc = 5, h1_size = 2, h2_size= 2, v_size= 2,
    ...
    # 2D test 
    out_blob = tbtest.make_feature_blob(np.zeros((10,10)), h1_loc=5, h2_loc=9, 
    h1_size = 2, h2_size= 4, shape='rectangle', amplitude= 1, PBC_flag='none')
    ...
    # 3D test 
    out_blob = tbtest.make_feature_blob(np.zeros((10, 10, 10)), h1_loc=5, h2_loc=9, 
                v_loc = 5, h1_size = 2, h2_size= 4, v_size= 2,
                 shape='rectangle', amplitude= 1, PBC_flag='none')
    ...
    for PBC_condition in ['hdim_1', 'hdim_2', 'both']:
        ...
        # 2D test
        out_blob = tbtest.make_feature_blob(np.zeros((10,10)), h1_loc=5, h2_loc=5, 
            h1_size = 2, h2_size= 2, shape='rectangle', amplitude= 1, PBC_flag=PBC_condition)
        assert np.all(out_blob[4:6, 4:6] == 1)
        # There should be exactly 4 points of value 1. 
        assert np.sum(out_blob) == 4 and np.min(out_blob) == 0
        ...
        out_blob = tbtest.make_feature_blob(np.zeros((10, 10, 10)), h1_loc=5, h2_loc=5, 
                    v_loc = 5, h1_size = 2, h2_size= 2, v_size= 2,
                    shape='rectangle', amplitude= 1, PBC_flag=PBC_condition)
        assert np.all(out_blob[4:6, 4:6, 4:6] == 1)
        # There should be exactly 8 points of value 1. 
        assert np.sum(out_blob) == 8 and np.min(out_blob) == 0
    ...
    for PBC_condition in ['hdim_2', 'both']:
        out_blob = tbtest.make_feature_blob(np.zeros((10,10)), h1_loc=5, h2_loc=9, 
        h1_size = 2, h2_size= 4, shape='rectangle', amplitude= 1, PBC_flag=PBC_condition)
        assert np.all(out_blob[4:6, 7:10] == 1)
...
def test_get_pbc_coordinates():
    '''Tests tobac.testing.get_pbc_coordinates. 
    Currently runs the following tests:
    ...
    with pytest.raises(ValueError):
        get_pbc_coordinates(0, 10, 0, 10, 1, 4, 1, 4, 'c')

    # Test PBC_flag of none

    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 1, 4, 'none') == [(1, 4, 1, 4),])
    assert (get_pbc_coordinates(0, 10, 0, 10, -1, 4, 1, 4, 'none') == [(0, 4, 1, 4),])
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 12, 1, 4, 'none') == [(1, 10, 1, 4),])
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 12, -1, 4, 'none') == [(1, 10, 0, 4),])

    # Test PBC_flag with hdim_1 
    # Simple case, no PBC overlapping
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 1, 4, 'hdim_1') == [(1, 4, 1, 4),])
    # PBC going on the min side
    assert (get_pbc_coordinates(0, 10, 0, 10, -1, 4, 1, 4, 'hdim_1') == [(0, 4, 1, 4), (9, 10, 1, 4)])
    # PBC going on the min side; should be truncated in hdim_2.
    assert (get_pbc_coordinates(0, 10, 0, 10, -1, 4, -1, 4, 'hdim_1') == [(0, 4, 0, 4), (9, 10, 0, 4)])
    # PBC going on the max side only
    assert (get_pbc_coordinates(0, 10, 0, 10, 4, 12, 1, 4, 'hdim_1') == [(4, 10, 1, 4), (0, 2, 1, 4)])
    # PBC overlapping
    assert (get_pbc_coordinates(0, 10, 0, 10, -4, 12, 1, 4, 'hdim_1') == [(0, 10, 1, 4),])

    # Test PBC_flag with hdim_2
    # Simple case, no PBC overlapping
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 1, 4, 'hdim_2') == [(1, 4, 1, 4),])
    # PBC going on the min side
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, -1, 4, 'hdim_2') == [(1, 4, 0, 4), (1, 4, 9, 10)])
    # PBC going on the min side with truncation in hdim_1
    assert (get_pbc_coordinates(0, 10, 0, 10, -4, 4, -1, 4, 'hdim_2') == [(0, 4, 0, 4), (0, 4, 9, 10)])
    # PBC going on the max side 
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 4, 12, 'hdim_2') == [(1, 4, 4, 10), (1, 4, 0, 2)])
    # PBC overlapping
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, -4, 12, 'hdim_2') == [(1, 4, 0, 10),])

    # Test PBC_flag with both
    # Simple case, no PBC overlapping
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 1, 4, 'both') == [(1, 4, 1, 4),])
    # hdim_1 only testing
    # PBC on the min side of hdim_1 only
    assert (get_pbc_coordinates(0, 10, 0, 10, -1, 4, 1, 4, 'both') == [(0, 4, 1, 4), (9, 10, 1, 4)])
    # PBC on the max side of hdim_1 only
    assert (get_pbc_coordinates(0, 10, 0, 10, 4, 12, 1, 4, 'both') == [(4, 10, 1, 4), (0, 2, 1, 4)])
    # PBC overlapping on max side of hdim_1 only
    assert (get_pbc_coordinates(0, 10, 0, 10, -4, 12, 1, 4, 'both') == [(0, 10, 1, 4),])
    # hdim_2 only testing
    # PBC on the min side of hdim_2 only
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, -1, 4, 'both') == [(1, 4, 0, 4), (1, 4, 9, 10)])
    # PBC on the max side of hdim_2 only
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, 4, 12, 'both') == [(1, 4, 4, 10), (1, 4, 0, 2)])
    #  PBC overlapping on max side of hdim_2 only
    assert (get_pbc_coordinates(0, 10, 0, 10, 1, 4, -4, 12, 'both') == [(1, 4, 0, 10),])
    # hdim_1 and hdim_2 testing simultaneous
    # both larger than the actual domain 
    assert (get_pbc_coordinates(0, 10, 0, 10, -1, 12, -4, 14, 'both') == [(0, 10, 0, 10),])
    # min in hdim_1 and hdim_2 
    assert (lists_equal_without_order(get_pbc_coordinates(0, 10, 0, 10, -3, 3, -4, 2, 'both'), [(0, 3, 0, 2), (0, 3, 6, 10), (7, 10, 6, 10), (7, 10, 0, 2)]))
    # max in hdim_1, min in hdim_2
    assert (lists_equal_without_order(get_pbc_coordinates(0, 10, 0, 10, 5, 12, -4, 2, 'both'), [(5, 10, 0, 2), (5, 10, 6, 10), (0, 2, 6, 10), (0, 2, 0, 2)]))
    # max in hdim_1 and hdim_2
    assert (lists_equal_without_order(get_pbc_coordinates(0, 10, 0, 10, 5, 12, 7, 15, 'both'), [(5, 10, 7, 10), (5, 10, 0, 5), (0, 2, 0, 5), (0, 2, 7, 10)]))
    # min in hdim_1, max in hdim_2
    assert (lists_equal_without_order(get_pbc_coordinates(0, 10, 0, 10, -3, 3, 7, 15, 'both'), [(0, 3, 7, 10), (0, 3, 0, 5), (7, 10, 0, 5), (7, 10, 7, 10)]))
...
def test_generate_single_feature():
    ...
    assert_frame_equal(generate_single_feature(1, 1, start_v = 1, 
                        ...
                        spd_h1 = 4, spd_h2 = 1, spd_v = 1, PBC_flag='hdim_1').sort_index(axis=1), expected_df.sort_index(axis=1))
    ...
    assert_frame_equal(generate_single_feature(1, 1, start_v = 1, 
                        min_h1 = 0, max_h1 = 10, min_h2 = 0, max_h2 = 10,
                        frame_start = 0, num_frames=4,
                        spd_h1 = 1, spd_h2 = 4, spd_v = 1, PBC_flag='hdim_2').sort_index(axis=1), expected_df.sort_index(axis=1))
