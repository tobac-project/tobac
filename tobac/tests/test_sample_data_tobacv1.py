...
def test_tracking_coord_order():
    ...
    segmentation_mask, features_segmentation = segmentation(
        Features, sample_data, dxy=dxy, **parameters_segmentation
    )
    segmentation_mask_inv, features_segmentation_inv = segmentation(
...
def test_tracking_3D():
    ...
    segmentation_mask_inv, features_segmentation = segmentation(
    ...
    # lifetime histogram:
    hist, bin_edges, bin_centers, minutes = lifetime_histogram(
        Track, bin_edges=np.arange(0, 200, 20), density=False, return_values=True
    )
    hist, bin_edges, bin_centers = lifetime_histogram(
        Track, bin_edges=np.arange(0, 200, 20), density=False, return_values=False
    )
    hist, bin_edges, bin_centers, minutes = lifetime_histogram(
        Track, bin_edges=np.arange(0, 200, 20), density=True, return_values=True
    )

    hist, bin_edges, velocities = velocity_histogram(
        Track,
    ...
    hist, bin_edges = velocity_histogram(
        Track,
        bin_edges=np.arange(0, 30, 1),
        density=False,
        method_distance=None,
    ...
    hist, bin_edges, distances = nearestneighbordistance_histogram(
        Features,
    ...
    hist, bin_edges = nearestneighbordistance_histogram(
        Features,
        bin_edges=np.arange(0, 30000, 500),
        density=False,
        method_distance=None,
    ...
    hist, bin_edges, bin_centers, areas = area_histogram(
        Features,
        segmentation_mask,
        bin_edges=np.arange(0, 30000, 500),
        density=False,
    ...
    hist, bin_edges, bin_centers, areas = area_histogram(
        ...
        method_area=None,
        return_values=True,
        representative_area=True,
    )

    hist, bin_edges, bin_centers = area_histogram(
        Features,
        segmentation_mask,
        bin_edges=np.arange(0, 30000, 500),
        density=False,
