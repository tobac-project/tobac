    ...
    for time_iteration_number, time_iteration_value in enumerate(
        ...
        if time_padding is not None:
            ...
            if isinstance(time_iteration_value.values.item(), int):
                min_time = time_iteration_value.values - pd.Timedelta(time_padding).to_timedelta64()
                max_time = time_iteration_value.values + pd.Timedelta(time_padding).to_timedelta64()
            else:
