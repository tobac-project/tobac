...
def to_cftime(dates: Union[str, datetime.datetime, np.datetime64, pd.Timestamp, cftime.datetime], calendar: str, align_on:str = "date") -> cftime.datetime:
    dates_arr = np.atleast_1d(dates)
    if isinstance(dates_arr[0], cftime.datetime):
        cftime_dates = xr.DataArray(
            dates_arr,
            xr.Coordinates({"time":dates_arr})
        ).convert_calendar(calendar, use_cftime=True, align_on=align_on).time.values
    else:
        cftime_dates = xr.DataArray(
            dates_arr,
            xr.Coordinates({"time":pd.to_datetime(dates_arr)})
        ).convert_calendar(calendar, use_cftime=True, align_on=align_on).time.values
    if not hasattr(dates, "__iter__") or isinstance(dates, str) and len(cftime_dates):
        return cftime_dates[0]
    return cftime_dates

def to_timestamp(dates: Union[str, datetime.datetime, np.datetime64, pd.Timestamp, cftime.datetime]) -> pd.Timestamp:
...
def to_datetime(dates: Union[str, datetime.datetime, np.datetime64, pd.Timestamp, cftime.datetime]) -> datetime.datetime:
    return to_timestamp(dates).to_pydatetime()

def to_datetime64(dates: Union[str, datetime.datetime, np.datetime64, pd.Timestamp, cftime.datetime]) -> np.datetime64:
    return to_timestamp(dates).to_numpy()

def to_datestr(dates: Union[str, datetime.datetime, np.datetime64, pd.Timestamp, cftime.datetime]) -> str:
